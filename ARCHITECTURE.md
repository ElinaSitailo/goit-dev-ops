...
┌─────────────────────────────────────────────────────────────┐
│  Developer Machine                                          │
│   git push  →  goit-dev-ops repo (main)                     │
└────────────────────────┬────────────────────────────────────┘
                         │ SCM trigger
                         ▼
┌─────────────────────────────────────────────────────────────┐
...
│   2. Push to ECR  (tag: <build>-<sha>  +  latest)          │
│   3. Update charts/django-app/values.yaml  in GitOps repo  │
│   4. git commit + push → GitOps repo (main)                │
└────────────────────────┬────────────────────────────────────┘
                         │ Git commit detected
                         ▼
┌─────────────────────────────────────────────────────────────┐
│  Argo CD  (EKS pod)                                        │
│   Watches: GitOps repo  branch: main  path: charts/django-app│
...
A developer pushes a commit to the `main` branch of the application repository (`goit-dev-ops`).

### Step 2 — Jenkins detects the change

Jenkins polls SCM. When a new commit is found it schedules a build and allocates a Kubernetes pod agent with four containers: `kaniko`, `git`, `yq`, and `aws-cli`. All containers share one workspace `emptyDir` volume.

### Step 3 — Checkout and tag calculation

The `Checkout Source` stage checks out the repository and computes a unique, traceable image tag:
...
3. Writes `$WORKSPACE/.docker/config.json` so Kaniko can authenticate without mounting `/var/run/docker.sock`.
...
```
Kaniko builds the image layer by layer without needing a Docker daemon (rootless). It pushes two tags: the versioned tag and `latest`.

### Step 6 — Update the GitOps repo

The `Update Helm values in GitOps repo` stage (runs in the `yq` container):
1. Clones the GitOps repository (`GITOPS_REPO_URL`) using the `gitops-repo-token` credential.
2. Updates the `image.tag` field in `charts/django-app/values.yaml`:
   ```
   yq e '.image.tag = strenv(IMAGE_TAG)' -i charts/django-app/values.yaml
   ```
...
2. **Pod template `kaniko-git`** — every pipeline build gets a pod with:

   | Container | Image | Purpose |
   |-----------|-------|---------|
   | `kaniko` | `gcr.io/kaniko-project/executor:v1.23.2-debug` | Build Docker image (rootless) |
   | `git` | `alpine/git:2.47.2` | Clone repos, commit, push |
   | `yq` | `mikefarah/yq:4.44.3` | Edit YAML values files |
...
Stage 3: Build and Push to ECR  (kaniko container)
  └─ /kaniko/executor --context app/ --dockerfile app/Dockerfile
     --destination <ecr>:<IMAGE_TAG>
     --destination <ecr>:latest

Stage 4: Update Helm values in GitOps repo  (yq + git containers)
...
- `timestamps()` — every log line is timestamped
